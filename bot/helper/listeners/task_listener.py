...
from ..mirror_leech_utils.uphoster_utils.buzzheavier_utils.upload import BuzzHeavierUpload
from ..mirror_leech_utils.uphoster_utils.pixeldrain_utils.upload import PixelDrainUpload
    ...
    async def on_download_start(self):
        ...
        if self.bot_pm and self.is_super_chat:
            self.pm_msg = await send_message(
                self.user_id,
                f"""➲ <b><u>Task Started (Professor-X) :</u></b>
┃
 ...
 """,
    ...
    async def on_upload_complete(
        ...
        if self.is_yt:
            ...
            if self.bot_pm:
                pmsg = msg
                pmsg += "〶 <b><u>Action Performed (Professor-X) :</u></b>\n"
    ...
    async def on_download_error(self, error, button=None, is_limit=False):
        ...
        msg = (
            f"""〶 <b><i><u>Limit Breached (Professor-X):</u></i></b>
│
┟ <b>Task Size</b> → {get_readable_file_size(self.size)}
┠ <b>In Mode</b> → {self.mode[0]}
┠ <b>Out Mode</b> → {self.mode[1]}
