...
async def gen_mediainfo(message, link=None, media=None, mmsg=None):
    temp_send = await send_message(message, "<i>Generating Professor-X MediaInfo...</i>")
    try:
        path = "mediainfo/"
    ...
    link_id = (await telegraph.create_page(title="Professor-X MediaInfo", content=tc))["path"]
    await temp_send.edit(
        f"<b>Professor-X MediaInfo:</b>\n\n➲ <b>Link :</b> https://graph.org/{link_id}",
