...
async def restart_bot(_, message):
    ...
    await send_message(
        message, "<i>Are you sure you want to restart the Professor-X Engine ?</i>", button
    )
...
async def restart_sessions(_, message):
    ...
    await send_message(
        message,
        "<i>Are you sure you want to restart the session(s) ?!</i>",
        button,
    )
    ...
    if Config.INCOMPLETE_TASK_NOTIFIER and Config.DATABASE_URL:
        if notifier_dict := await database.get_incomplete_tasks():
            for cid, data in notifier_dict.items():
                msg = f"""⌬ <b><i>{"Professor-X Restarted Successfully!" if cid == chat_id else "Professor-X Engine Restarted!"}</i></b>
    ...
    if await aiopath.isfile(".restartmsg"):
        try:
            await TgClient.bot.edit_message_text(
                chat_id=chat_id,
                message_id=msg_id,
                text=f"""⌬ <b><i>Professor-X Restarted Successfully!</i></b>
┟ <b>Date:</b> {now.strftime("%d/%m/%y")}
┠ <b>Time:</b> {now.strftime("%I:%M:%S %p")}
┠ <b>TimeZone:</b> Asia/Kolkata
┖ <b>Version:</b> {get_version()}""",
...
async def confirm_restart(_, query):
    ...
    if data[1] == "confirm":
        ...
        restart_message = await send_message(reply_to, "<i>Restarting Professor-X Engine...</i>")
        await delete_message(message)
        await TgClient.stop()
