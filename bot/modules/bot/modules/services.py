...
async def start(_, message):
    ...
    buttons = ButtonMaker()
    buttons.url_button(
        lang.START_BUTTON1, "https://github.com/Professor-X/Professor-X-Bot"
    )
    buttons.url_button(lang.START_BUTTON2, "https://t.me/Professor_X")
    ...
    if await CustomFilters.authorized(_, message):
        ...
        await send_message(
            message,
            "<i>Professor-X Engine is ready. Bot will send you all your files and links here...</i>",
            reply_markup,
        )
    else:
        await send_message(
            message,
            "<i>Bot can mirror/leech from links|tgfiles|torrents|nzb|rclone-cloud to any rclone cloud, Google Drive or to telegram.\n\n⚠️ You Are not an authorized user! Deploy your own Professor-X Engine</i>",
            reply_markup,
        )
...
async def login(_, message):
    if Config.LOGIN_PASS is None:
        ...
        if Config.DATABASE_URL:
            await database.update_user_data(user_id)
        return await send_message(
            message, "<b>Professor-X Engine Permanently Unlocked!</b>\n\n<i>You now have full access.</i>"
        )
    else:
        await send_message(
...
async def ping(_, message):
    ...
    reply = await send_message(message, "<i>Starting Professor-X Ping..</i>")
    end_time = monotonic()
    await edit_message(
