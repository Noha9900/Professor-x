...
def wrap_with_retry(obj, max_retries=5): # PROFESSOR-X: Increased retries to handle aggressive downloads
    for attr_name in dir(obj):
        ...
        if iscoroutinefunction(attr):
            retry_policy = retry(
                ...
                wait=wait_exponential(multiplier=0.5, min=1, max=3), # PROFESSOR-X: Faster retry attempts
...
class TorrentManager:
    ...
    async def initiate(cls):
        ...
        try:
            ...
            LOGGER.info("Professor-X Aria2 initialized successfully.")
