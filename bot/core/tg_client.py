...
class TgClient:
    ...
    @classmethod
    def profxClient(cls, *args, **kwargs):
    ...
    async def start_hclient(cls, no, b_token):
        try:
            hbot = await cls.profxClient(
                f"ProfX-HBot{no}",
                bot_token=b_token,
                no_updates=True,
            ).start()
            LOGGER.info(f"Professor-X Helper Bot [@{hbot.me.username}] Started!")
    ...
    async def start_helper_bots(cls):
        if not Config.HELPER_TOKENS:
            return
        LOGGER.info("Generating Professor-X helper clients from HELPER_TOKENS")
    ...
    async def start_bot(cls):
        LOGGER.info("Generating Professor-X client from BOT_TOKEN")
        cls.ID = Config.BOT_TOKEN.split(":", 1)[0]
        cls.bot = cls.profxClient(
            f"ProfX-Bot{cls.ID}",
        ...
        cls.ID = Config.BOT_TOKEN.split(":", 1)[0]
        LOGGER.info(f"Professor-X Bot : [@{cls.BNAME}] Started!")

    @classmethod
    async def start_user(cls):
        if Config.USER_SESSION_STRING:
            LOGGER.info("Generating Professor-X client from USER_SESSION_STRING")
            try:
                cls.user = cls.profxClient(
                    "ProfX-User",
                ...
                LOGGER.info(f"Professor-X User : [{uname}] Started!")
            except Exception as e:
                LOGGER.error(f"Failed to start Professor-X user from USER_SESSION_STRING. {e}")
    ...
    async def stop(cls):
        async with cls._lock:
            ...
            if cls.helper_bots:
                ...
                cls.helper_bots = {}
            LOGGER.info("All Professor-X Client(s) stopped")
    ...
    async def reload(cls):
        async with cls._lock:
            ...
            LOGGER.info("All Professor-X Client(s) restarted")
